...
static int MPG123_open(Sound_Sample *sample, const char *ext)
{
    ...
    */
    ...
    SNDDBG(("MPG123: Accepting data stream.: %p\n", internal->rw));
...
mpg123_open_failed:
    ...
    SNDDBG(("MPG123: Open return error.\n"));
    return(0);
} /* MPG123_open */
...
static Uint32 MPG123_read(Sound_Sample *sample)
{
    ...
    SNDDBG(("MPG123: Read (%d=%d): %p\n", internal->buffer_size, bw, internal->rw));
    if (rc == MPG123_DONE) {
...
static int MPG123_seek(Sound_Sample *sample, Uint32 ms)
{
    ...
    const off_t rc = /*(int)*/ mpg123_seek(mp, frame_offset , SEEK_SET);
    const off_t pos = mpg123_tell(mp);
SNDDBG(("MPG123_seek %u ms, offset=%ld pos=%ld, return %ld\n", ms, frame_offset, pos, rc));
    BAIL_IF_MACRO(rc < 0, set_error(mp, rc), 0);
    return(1);
} /* MPG123_seek */
